...
describe GameAr do
  ...
  %W{positive negative}.each do |posneg|
    describe "#array_#{posneg}_reviews" do
      before :each do
        # We do this so we don't have to repeat the tests twice
        game.class.send :alias_attribute, :posneg_reviews, :"#{posneg}_reviews"
        game.class.send :alias_attribute, :average_time_posneg, :"average_time_#{posneg}"
      end

      describe 'validations' do
        it 'is always an array' do
          game.posneg_reviews = nil
          game.posneg_reviews.should eq []
        end

        it 'accepts an actual array' do
          game.posneg_reviews = [1,2,3]
          game.should be_valid
        end

        it 'does not accepts an string in the array' do
          game.posneg_reviews = [1,'h', 3, 4]
          game.should_not be_valid
        end

        it 'does accept a float in the array' do
          game.posneg_reviews = [1,1.2, 3, 4]
          game.should be_valid
        end

        it 'does not accepts time in the array' do
          game.posneg_reviews = [1,Time.now, 3, 4]
          game.should_not be_valid
        end
      end
      ...
      describe 'computated methods' do
        context 'array is valid' do 
          it "#average_time_#{posneg} returns the average time for the #{posneg}" do
            game.posneg_reviews = [1,3,4,7]
            game.average_time_posneg.should eq (1+3+4+7)/4.0
          end

          context 'array is empty' do
            it "#average_time_#{posneg} should be 0" do
              game.posneg_reviews = []
          ...
          it '#max_time returns the maximum time' do
            game.posneg_reviews = [1,5,1,2,12,3]
